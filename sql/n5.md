...
Este conjunto de ejercicios está diseñado para practicar funciones escalares usando las tablas `products`, `customers`, `offices` y `payments`. Todos los ejercicios se pueden resolver con los conocimientos adquiridos en la unidad.
---
...
| Columna | Descripción |
|---------|-------------|
| `officeCode` | Código de la oficina |
| `city` | Ciudad |
| `country` | País |
...
## Ejercicio 11: COALESCE con una y múltiples columnas

**Enunciado:**
Escribe dos consultas sobre la tabla `offices`. En la primera, muestra el código de oficina y la segunda línea de dirección (`addressLine2`), pero si es nula, muestra "Sin segunda dirección". En la segunda, muestra el código de oficina con una descripción de ubicación que use primero `addressLine2`, si no existe usa `state`, y si tampoco existe usa "Ubicación desconocida".
...
**Solución:**
```sql
-- Con un solo valor alternativo
SELECT officeCode, COALESCE(addressLine2, "Sin segunda dirección") AS "Segunda Dirección"
FROM offices;

-- Con múltiples valores alternativos
SELECT officeCode, COALESCE(addressLine2, state, "Ubicación desconocida") AS "Ubicación"
FROM offices;
```

**Explicación:**
La función `COALESCE()` devuelve el primer valor que no sea nulo. En la primera consulta, si `addressLine2` tiene un valor lo muestra, y si es nulo muestra "Sin segunda dirección". En la segunda, si `addressLine2` es nulo intenta con `state`, y si ambos son nulos devuelve "Ubicación desconocida". Es muy útil para manejar datos faltantes y garantizar que siempre haya un valor legible en los resultados.

</details>
---

## Ejercicio 12: Combinación de funciones escalares
...
## Ejercicio 13: ROUND con agregado
...
## Ejercicio 15: SUBSTRING con LIKE
...
## Ejercicio 17: LOWER para normalizar búsquedas
