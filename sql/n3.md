...
**Enunciado:**
Crea una consulta que muestre el nombre del cliente (`customerName`) y el estado (`state`) de clientes en la ciudad de "New York" (NY en la base de datos), ordenados por nombre de cliente de forma ascendente.
...
**Solución:**
```sql
SELECT customerName, state FROM customers WHERE state = 'NY' ORDER BY customerName;
```
...
Ordena por país y por ciudad en orden ascendente. Muestra solo los primeros 10 resultados.
...
**Solución:**
```sql
SELECT customerName, country, city, creditLimit FROM customers WHERE country <> 'USA' AND creditLimit >= 50000 ORDER BY country ASC, city ASC LIMIT 10;
```

**Explicación:**
Filtra por país diferente a USA y crédito alto, luego ordena por tres niveles (país, ciudad y crédito) y limita a 10 resultados.
